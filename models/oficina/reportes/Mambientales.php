...
EOF;
    ...
    public function getPruebagases($idhojapruebas)
    {
        $query = $this->db->query("SELECT p.idprueba, p.idmaquina,
                                    ...
                                    WHERE 
                                    v.idvehiculo = h.idvehiculo AND 
                                    p.idhojapruebas = h.idhojapruebas 
                                    AND (p.idtipo_prueba=3 or p.idtipo_prueba = 2) 
                                    AND (p.estado <> 0 AND p.estado <> 5 AND p.estado <> 9) AND p.idhojapruebas=$idhojapruebas AND p.idmaquina IS NOT NULL
                                    ");
        if ($query->num_rows() > 0) {
            $query = $query->result();
            return $query;
    ...
    public function ntc5365($idmaquina, $idprueba)
    {
        ...
        $consulta = <<<EOF
            SELECT 
                ...
                IFNULL((SELECT ci.nombre FROM sede se,  ciudades ci WHERE se.cod_ciudad = ci.cod_ciudad LIMIT 1),'') AS 'ciudad',
                IFNULL((SELECT se.direccion FROM sede se LIMIT 1),'') AS 'direccion', 
...
CASE 
    ...
    ELSE 
        IFNULL((SELECT nl.idlineas FROM newlineas nl WHERE nl.idlineas=v.idlinea LIMIT 1),'SIN LINEA')
END as linea,
CASE 
    WHEN v.migrateLineaMarca <> 1 THEN
